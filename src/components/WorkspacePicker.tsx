...
export default function WorkspacePicker({ onSelect, onSkip, currentDir, showSkip = true }: WorkspacePickerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-700 rounded-lg shadow-2xl p-6 max-w-md w-full mx-4">
        <h2 className="text-sm font-pixel text-indigo-300 mb-1">Choose Working Directory</h2>
        ...
        {currentDir && (
          <div className="mb-4 bg-slate-800 rounded p-2">
            <p className="text-[10px] font-pixel text-slate-500 mb-0.5">Current directory</p>
            <p className="text-[11px] font-mono text-slate-300 truncate">{currentDir}</p>
          </div>
        )}
